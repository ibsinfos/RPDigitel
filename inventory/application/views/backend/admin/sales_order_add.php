...
<?php echo form_open(base_url() . 'index.php?admin/sales_order/add/' , array(
		'class' => 'form-horizontal form-groups' , 'enctype' => 'multipart/from-data' , 'id' => 'validated_form'));?>
			...
			<div class="panel-body">
				...
				<div class="col-sm-5">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<select name="customer_user_id" class="select2" id="customer"
								onchange="get_address(this.value)" required>
								<option value=""><?php echo get_phrase('select_a_customer');?></option>
								<?php
									$customers = $this->info_model->get_users(2);
...
<script type="text/javascript">
	...
	{
		$.ajax({
			url: '<?php echo base_url();?>index.php?admin/reload_customer_address/' + user_id,
	...
	{
		$.ajax({
			url: '<?php echo base_url();?>index.php?admin/sales_order_entry_response/' + variant_id,
			success: function(response)
			{
	...
	function append_sales_order_entry()
	{
		...
		count++;
		$.ajax({
			url: '<?php echo base_url();?>index.php?admin/sales_order_append_entry_response/' + count + '/' + selected_variants,
			success: function(response)
			{
...
<script type="text/javascript">
	function show_alert()
	{
		if($('#order_code').val() == '' || $('#customer').val() == '' || $('#billing_address_id').val() == '' || $('#shipping_address_id').val() == '')
			toastr.error('<?php echo get_phrase("please_insert_order_code,_customer,_billing_address_and_shipping_address"); ?>');
		else
