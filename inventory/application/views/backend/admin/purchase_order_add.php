...
<?php echo form_open(base_url() . 'index.php?admin/purchase_order/add/' , array(
				...
				<div class="col-sm-7">
					...
					<div id="selected_supplier_address">
						...
						<div class="form-group">
							...
							<div class="col-sm-7">
								<select name="" class="selectboxit" id="" required="true">
...
<center>
	...
	<button type="submit" id="" class="btn btn-green btn-icon icon-left" onclick="show_alert()">
...
<script type="text/javascript">
	...
	{
		$.ajax({
			url: '<?php echo base_url();?>index.php?admin/reload_supplier_address/' + user_id,
	...
	{
		$.ajax({
			url: '<?php echo base_url();?>index.php?admin/purchase_order_entry_response/' + variant_id,
			success: function(response)
			{
	...
	function append_purchase_order_entry()
	{
		...
		count++;
		$.ajax({
			url: '<?php echo base_url();?>index.php?admin/purchase_order_append_entry_response/' + count + '/' + selected_variants,
			success: function(response)
			{
	...
	function show_alert()
	{
		if($('#supplier').val() == '' || $('#supplier_address_id').val() == '')
			toastr.error('<?php echo get_phrase("please_insert_supplier_and_supplier_address"); ?>');
	}
