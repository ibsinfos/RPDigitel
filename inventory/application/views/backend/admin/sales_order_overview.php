...
<div class="row">
	...
	<?php endif;?>
		<a href="<?php echo base_url();?>index.php?admin/sales_order_view/<?php echo $row['order_code'];?>/overview" style="text-align: left;"
			class="btn btn-<?php if($view_page == 'sales_order_overview') echo 'primary'; else echo 'default';?> btn-icon icon-left btn-block <?php if($row['order_status'] == 0) echo 'disabled';?> text-left">
	...
	<div class="col-md-10">
		...
		<div class="col-sm-3">
			<?php
				...
				$payments_of_order = $this->db->get_where('payment' , array('order_id' => $row['order_id']))->result_array();
				$paid_amount = 0;
				foreach($payments_of_order as $order_payment) {
					$amount = $this->db->get_where('invoice' , array('invoice_id' => $order_payment['invoice_id'], 'order_type' => 'sales'))->row()->total_amount;
					$paid_amount += $amount;
				}
			?>
					...
					<table class="table table-bordered table-responsive">
						...
						?>
							<tr>
								...
								<td>
									<a href="<?php echo base_url();?>index.php?admin/sales_order_payment_print_view/<?php echo $payment['payment_id'];?>"
										class="btn btn-default btn-xs tooltip-primary" target="_blank"
											data-toggle="tooltip" data-original-title="<?php echo get_phrase('print');?>">
										<i class="entypo-print" style="color: #949494;"></i>
									</a>
