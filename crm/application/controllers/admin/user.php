		...
		public function save_user()
		{
			...
			if (!empty($check_user) || !empty($check_email)) { // if input data already exist show error alert
				...
				*/
				...
				/*Added by Ranjit on 21st April 2017 to check users limit Start*/
				if(!isset($_SESSION)) {
					session_start();
				}
                ...
                $select_user_count= "select no_of_registered_users,max_allowed_users from database_details where id='".$_SESSION['crm_db_id']."';";
                $result_user_count = mysqli_query($rpcon, $select_user_count) or die($rpcon->error);
				if($result_user_count){
					while($row=mysqli_fetch_assoc($result_user_count)){
						$no_of_registered_users=$row['no_of_registered_users'];
						$max_allowed_users=$row['max_allowed_users'];
					}
				}
				
				if(isset($no_of_registered_users) && isset($max_allowed_users)){
					$rem_users_to_add=$max_allowed_users-$no_of_registered_users;
					if($rem_users_to_add>0){
						...
						/* Added Ranjit Pasale on 24 April 2017 to register user with RPDigitel Databse Start*/
						
						//update registered users count
						
						$no_of_registered_users++;
						$up_user_count= "update database_details set no_of_registered_users='".$no_of_registered_users."' where user_id='".$_SESSION['crm_db_id']."'"; 
						$result = mysqli_query($rpcon, $up_user_count) or die($rpcon->error);
						
						
						$ins_rp_user_query = "INSERT INTO membership (phone_no,email_address,username,password,role,crm_db_id) VALUES ('".$mobile."', '".$login_data['email']."', '".$login_data['username']."', '".$login_data['password']."', 'user','".$_SESSION['crm_db_id']."')";                
						$result_rpuser = mysqli_query($rpcon, $ins_rp_user_query) or die($rpcon->error);
						
						
						//Added by Ranjit on 26 April 2017 to register user with novaecard(tbl_users) Start
						
						$ins_vc_user_query = "INSERT INTO tbl_users (email,first_name,mobile,verified,email_verify,password) VALUES ('".$login_data['email']."', '".$login_data['username']."', '".$mobile."', '1', '1','".$login_data['password']."')";
						$result_vcuser = mysqli_query($rpcon, $ins_vc_user_query) or die($rpcon->error);
						
						//Added by Ranjit on 26 April 2017 to register user with novaecard(tbl_users) End
						
						
						
						$select_last_inserted_userid= "select user_id from membership where username='".$login_data['username']."';";
						$result_last_inserted_userid = mysqli_query($rpcon, $select_last_inserted_userid) or die($rpcon->error);
						if($result_last_inserted_userid){
							while($row=mysqli_fetch_assoc($result_last_inserted_userid)){
								$user_id=$row['user_id'];
							}
							}else{
							$$user_id=0;
						...
						$ins_mem_serv_query = "INSERT INTO member_services (user_id, service_id, free_trial_start_date, free_trial_period_in_days, free_trial_end_date) VALUES ('".$user_id."', '1', '".date("Y-m-d")."', '".$free_days."', '".date('Y-m-d', strtotime("+" . $free_days . " days"))."')";                
						$result_mem_serv = mysqli_query($rpcon, $ins_mem_serv_query) or die($rpcon->error);
						
						
						/* Added Ranjit Pasale on 24 April 2017 to register user with RPDigitel Databse End*/
						
						
						
						}else{
						set_message('error', 'You have already added '.$max_allowed_users.' users');
						redirect($_SERVER['HTTP_REFERER']);
					}
					}else{
					...
					/*Added by Ranjit on 21st April 2017 to check users limit End*/
					...
					/* Added Ranjit Pasale on 24 April 2017 to Delete user with RPDigitel Database Start*/
					...
					//update registered users count(decreament by 1)
					$up_user_count= "update database_details set no_of_registered_users=no_of_registered_users-1 where id='".$_SESSION['crm_db_id']."';";                
					$result = mysqli_query($rpcon, $up_user_count) or die($rpcon->error);
					
					//Delete from membership and member_services table Start
					
					$q_select_user_data= "select user_id from membership where username='".$login_data['username']."';";
					$select_user_data= mysqli_query($rpcon, $q_select_user_data) or die($rpcon->error);
					if($select_user_data){
					while($row=mysqli_fetch_assoc($select_user_data)){
                    $user_id_delete=$row['user_id'];
					}
					
					
					$query_delete_user_data= "delete from membership where user_id='".$user_id_delete."'";
					$delete_user_data= mysqli_query($rpcon, $query_delete_user_data) or die($rpcon->error);
					
					$query_delete_crm_membership= "delete from member_services where user_id='".$user_id_delete."'";
					$delete_crm_membership= mysqli_query($rpcon, $query_delete_crm_membership) or die($rpcon->error);
					
					
					}
					
					//Delete from membership and member_services table End
					
					
					
					/* Added Ranjit Pasale on 24 April 2017 to Delete user with RPDigitel Database End*/
					...
					//Update RPDigitel User Status as active  
					$type = "success";
