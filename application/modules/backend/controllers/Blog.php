...
//        $this->load->view('front/includes/header-2', $data);
    ...
    public function addComment($post_id = '')
    {
        ...
        if (!$this->common_model->isLoggedIn()) {
            $this->session->set_userdata('error_msg', $this->lang->line('Please_login_to_proceed_further'));


            redirect(base_url() . 'blog-details/' . $post_id);
        }
        $data['user_session'] = $this->session->userdata('user_account');
        ...
        $last_insert_id = $this->common_model->insertRow($arr_to_insert, "trans_blog_comments");
        $this->session->set_userdata('success_msg', $this->lang->line('Your_comment_has_been_posted_successfully'));
        redirect(base_url() . 'blog-details/' . $post_id);
    }
...
//        $this->template->set('blog_posts',$data['blog_posts']);
        ...
        $this->template->set_theme('default_theme');
        $this->template->set_layout('backend_silo')
                ->title($data["title"].' | '.$this->global_setting['site_title'])
                ->set_partial('header', 'partials/header')
                ->set_partial('sidebar', 'partials/sidebar')
                ->set_partial('footer', 'partials/footer');
    ...
    public function view_post_comments($post_id)
    {
        ...
        $this->template->set_theme('default_theme');
        $this->template->set_layout('backend_silo')
                ->title('Comments | '.$this->global_setting['site_title'])
                ->set_partial('header', 'partials/header')
                ->set_partial('sidebar', 'partials/sidebar')
                ->set_partial('footer', 'partials/footer');
