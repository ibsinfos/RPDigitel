...
Class News extends MX_Controller {
    ...
    public function newsDetail($id) {
        ...
        $recent_news = $this->common_model->getRecords(TABLES::$NEWS, '*', '', 'id desc', '2');
        $this->template->set('news', $news[0]);
        $this->template->set('recent_news', $recent_news);
        $this->template->set('page', 'event');
        $this->template->set_theme('default_theme');
        $this->template->set_layout('backend_community')
                ->title($news[0]['title'] . ' | Silo')
