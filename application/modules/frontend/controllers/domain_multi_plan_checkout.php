...
class domain_multi_plan_checkout extends CI_Controller {
...
// Set example cart data into session
        ...
        $this->load->view('paypal/demos/domain_multi_plan_checkout/index', $cart);
    }
    ...
    function SetExpressCheckout() {
        ...
        $SECFields = array(
            ...
            'returnurl' => site_url('frontend/domain_multi_plan_checkout/GetExpressCheckoutDetails'), // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.
            'cancelurl' => site_url('frontend/domain_multi_plan_checkout/OrderCancelled'), // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            $this->load->view('paypal/demos/domain_multi_plan_checkout/paypal_error');
        } else {
    ...
    function GetExpressCheckoutDetails() {
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            $data['main_content'] = 'paypal/demos/domain_multi_plan_checkout/paypal_error';
            $this->load->view('includes/template', $data);
            $this->template->build('paypal/demos/domain_multi_plan_checkout/paypal_error');
        } else {
            // Successful call.
	    ...
	    $this->load->model('subscription_model');
            ...
            if ($payment_data['ack'] == 'Success') {
                $orderId = $this->session->userdata('domain_order_id');
                $accept_order = $this->acceptOrder($orderId);
                // redirect('http://'.$_SERVER['SERVER_NAME'].'/crm/login');
                
		$this->session->set_userdata('payment_successfull', '1');
                redirect(base_url() . 'domain_check_out');
    ...
    function DoExpressCheckoutPayment() {
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            // Successful Order
            redirect('frontend/domain_multi_plan_checkout/OrderComplete');
        }
    }
    
    public function acceptOrder($orderId)
    {
       // echo 'hi22'.$orderId;die;
        try {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'http://tweebrhost.com/includes/api.php');
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS,
                http_build_query(
                    [
                        'action'       => 'AcceptOrder',
                        'username'     => 'tweebrwebmaster',
                        'password'     => md5('Life_rocks4'),
                        'orderid'      => $orderId,
                        'registrar'    => 'enom',
                        'autosetup'    => true,
                        'sendemail'    => true,
    ...
    function OrderComplete() {
        ...
        if (empty($cart))
            redirect('frontend/domain_multi_plan_checkout');
        ...
        $this->template->build('paypal/demos/domain_multi_plan_checkout/payment_complete');
    ...
    function OrderCancelled() {
        ...
        $this->template->set_layout('frontend')
                ...
                ->set_partial('header', 'partials/header')
                ->set_partial('footer', 'partials/footer');
        $this->template->build('paypal/demos/domain_multi_plan_checkout/order_cancelled');
    }
    ...
    public function addOrder()
    {
        ...
        //echo '<pre>';print_r($contents);die;
            try {
                // credentials Array containing credentials informations with action attribute.
                $credentials = [
                    'action'        => 'AddOrder',
...
//                    'password'      => md5($this->password),
                ...
                //domainData Array contains all the items of cart
                $i = 0;
                foreach ($contents  as $content) {
                    $domainData[] = [
                        'domain[' . $i . ']'       => $content['name'],
                        'billingcycle[' . $i . ']' => 'monthly',
                        'domaintype[' . $i . ']'   => 'register',
                        'regperiod[' . $i . ']'    => $content['qty'],
                    ];
                    $i++;
                }
                ...
                if(count($domainData) > 0){
                    $newArray = call_user_func_array("array_merge", $domainData);
                }
                $domains = array_merge($credentials, $newArray);
                // Initialize the curl and pass the required data.
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'http://tweebrhost.com/includes/api.php');
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $domains);
                $response = curl_exec($ch);
                $arr_response = json_decode($response, true);
               // echo '<pre>';print_r(json_decode($response, true));die;
                $this->session->unset_userdata('domain_order_id');
                $this->session->set_userdata('domain_order_id',($arr_response['orderid']));
                curl_close($ch);
            } catch (Exception $e) { 
                return response()->json([
                    'status'  => 'error',
