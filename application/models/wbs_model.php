	...
	class Wbs_model extends CI_Model {
		...
		function get_free_trial($service_id, $days, $org_name) {
			...
			if ($chk_free_trial->num_rows() > 0) {
                ...
                'no_of_registered_users' => $no_of_registered_users
				);
				
				$ins_database_details = $this->db->insert('database_details', $data_database_details);
				//insert into database_tables table end
				
				//update membershipdetails table with crm db id Start
				
				$crm_db_id=$this->db->insert_id();
				
				//if user has not registered through crm appln and clicked on crm free trial
				if(!isset($_SESSION)) {
					session_start();
				}
				
				$_SESSION['crm_db_id']=$crm_db_id;
				
				$crm_db_data = array('crm_db_id' => $crm_db_id);
