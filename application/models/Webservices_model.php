<?php
    ...
    class Webservices_model extends CI_Model
    {
		...
		{		
			...
			if(!empty($this->input->post('username')) && !empty($this->input->post('password'))) {
				...
				if($sel_user->num_rows>0){
					...
					$result['message']="Enter Correct Username, Password";
					$result['status']=false;
					
				}		
				
				} else {
				$result['message']="Enter Username, Password";
				$result['status'] = false;
			}
			return $result;
		}
		
		public function socialLogin($post)
		{
			$result = array();
			$result['data'] = array();
			
			// if(!empty($this->input->post('name')) && !empty($this->input->post('email')) && !empty($this->input->post('phone_no')) && !empty($this->input->post('social_unique_id')) && !empty($this->input->post('deviceid')) && !empty($this->input->post('prifile_pic')) && !empty($this->input->post('social_type'))) {
			if(!empty($this->input->post('social_unique_id'))) {
				
				$social_unique_id=$this->input->post('social_unique_id');
				// $password=str_ireplace(array("\r","\n",'\r','\n'),'', $this->input->post('password'));
				// $username=stripslashes($this->input->post('username'));
				// $password=stripslashes($this->input->post('password'));
				...
				$q="SELECT `user_id`, `first_name`, `last_name`, `phone_no`, `email_address`, `username` FROM (`membership`) WHERE social_unique_id ='".$social_unique_id."'";				
				// $result['query']=$q;
				...
				if($sel_user->num_rows>0){
					...
					$result['message']="Logged in Successfully";
					$result['status']=true;
					$result['SocialAvailable']=true;
					
					}else{
					
					$result['message']="Record not found";
					$result['status']=true;
					$result['SocialAvailable']=false;
					
				}		
				
				} else {
				$result['message']="Please login with social login";
				$result['status'] = false;
			}
			return $result;
		}
		...
		public function socialRegister($post)
		{
			$result = array();
			$result['data'] = array();
			
			if(!empty($this->input->post('deviceid')) && !empty($this->input->post('deviceid'))) {
				...
				$result['message'] = "Sign up Successfull";
				$result['status']=true;
				...
				$result['message'] = "Enter all required information";
				$result['status'] = false;
			}
			return $result;
		}
		...
		public function registerUser($post)
		{
			...
			if(!empty($this->input->post('username')) && !empty($this->input->post('email_address')) && !empty($this->input->post('password')) && !empty($this->input->post('deviceid'))) {
					...
					foreach( $files as $key => $file ) {
						...
						foreach($fileDir1 as $value) {
							if (strpos($value, '.') !== false) {
								...
								$fileDetailPath1 = 'uploads/'.$post['userid'].'/SiloSd/'.$file_path1.'/'.$file['name'];
							}
						}	
							...
							foreach($fileDir as $value) {
								...
								if (strpos($value, '.') === false) {
									if($cnt == 0 ) {
										...
										if (!is_dir('/uploads/'.$post['userid'].'/SiloSd/'.$value)) {
											mkdir('./uploads/'.$post['userid'].'/SiloSd/' . $value, 0777, TRUE);
											
										} 
										} else {
										if (!is_dir('/uploads/'.$post['userid'].'/SiloSd/'.$fileDir[$cnt-1]. '/'. $value)) {
											mkdir('./uploads/'.$post['userid'].'/SiloSd/' .$fileDir[$cnt-1]. '/'. $value, 0777, TRUE);
										} 
									}
									...
									$fileDetailPath = 'uploads/'.$post['userid'].'/SiloSd/'.$file_path.'/'.$file['name'];									
									$config['file_name'] = $file['name'];
									$config['upload_path'] = 'uploads/'.$post['userid'].'/SiloSd/'.$file_path.'/';
									...
									if ($this->upload->do_upload($key)) {  
										...
										"file_name"=> 'uploads/'.$post['userid'].'/SiloSd/'.$file_path.'/'.$image_path
										);
		...
		public function getUserFiles( $post ) {
			...
			if( isset( $post['userid'] ) && !empty($post['userid'])) {
				...
				foreach( $file as $value ) {
					...
					$this->db->from('tbl_files');
					$this->db->where('user_id', $post['userid']);
					$this->db->where('file_name', 'uploads/'.$post['userid'].'/SiloSd/'.$value);
