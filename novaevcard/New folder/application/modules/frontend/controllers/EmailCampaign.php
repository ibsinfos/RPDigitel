		...
		public function send_email() {
			...
			*/
			if ($this->input->post('send_time') == 'send_now') {
				...
				if (isset($_POST['list_name'])) {
					foreach ($this->input->post('list_name') as $opt) {
						// $contact_email = $this->common_model->getRecords(TABLES::$EMAIL_MAPPING, "contact_id", array('list_id' => $opt));
						$contact_email = $this->common_model->getRecords(TABLES::$CONTACT_MAPPING, "contact_id", array('list_id' => $opt));
						foreach ($contact_email as $key => $c) {
							// $email_id = $this->common_model->getRecords(TABLES::$CLIENT_EMAIL_IDS, "email", array('id' => $c['contact_id']));
							$email_id = $this->common_model->getRecords(TABLES::$CLIENT_CONTACTS, "email", array('id' => $c['contact_id']));
