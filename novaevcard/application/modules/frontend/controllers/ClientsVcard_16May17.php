...
class ClientsVcard extends CI_Controller {
    ...
    public function index() {
        ...
        $arr = $this->common_model->getRecords(TABLES::$ADMIN_USER, '*', array('slug' => $slug));
        if (count($arr) < 1) {
        ...
        $this->template->title(ucfirst($arr[0]['first_name']) . " " . ucfirst($arr[0]['last_name']) . " | " . ucfirst($arr[0]['company_name']));
        $this->template->build('client_vcard');
    }
