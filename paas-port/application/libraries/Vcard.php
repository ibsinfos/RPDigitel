...
class Vcard
{
    ...
    private function _build()
    {
        ...
        if ($this->data['nickname']) {
            $this->card_string .= "NICKNAME:" . $this->data['nickname'] . "\r\n";
        }
        if ($this->data['title']) {
            $this->card_string .= "TITLE:" . $this->data['title'] . "\r\n";
        }
        if ($this->data['company']) {
            $this->card_string .= "ORG:" . $this->data['company'];
        }

        if ($this->data['department']) {
            $this->card_string .= ";" . $this->data['department'];
        }
        $this->card_string .= "\r\n";
        ...
        if ($this->data['email1']) {
            $this->card_string .= "EMAIL;TYPE=internet,pref:" . $this->data['email1'] . "\r\n";
        }
        if ($this->data['email2']) {
            $this->card_string .= "EMAIL;TYPE=internet:" . $this->data['email2'] . "\r\n";
        }
        if ($this->data['office_tel']) {
            $this->card_string .= "TEL;TYPE=work,voice:" . $this->data['office_tel'] . "\r\n";
        }
        if ($this->data['home_tel']) {
            $this->card_string .= "TEL;TYPE=home,voice:" . $this->data['home_tel'] . "\r\n";
        }
        if ($this->data['cell_tel']) {
            $this->card_string .= "TEL;TYPE=cell,voice:" . $this->data['cell_tel'] . "\r\n";
        }
        if ($this->data['fax_tel']) {
            $this->card_string .= "TEL;TYPE=work,fax:" . $this->data['fax_tel'] . "\r\n";
        }
        if ($this->data['pager_tel']) {
            $this->card_string .= "TEL;TYPE=work,pager:" . $this->data['pager_tel'] . "\r\n";
        }
        if ($this->data['url']) {
            $this->card_string .= "URL;TYPE=work:" . $this->data['url'] . "\r\n";
        }
        if ($this->data['gender']) {
            $this->card_string .= "GENDER:" . $this->data['gender'] . "\r\n";
        }
        if ($this->data['birthday']) {
            $this->card_string .= "BDAY:" . $this->data['birthday'] . "\r\n";
        }
        if ($this->data['role']) {
            $this->card_string .= "ROLE:" . $this->data['role'] . "\r\n";
        }
        if ($this->data['note']) {
            $this->card_string .= "NOTE:" . $this->data['note'] . "\r\n";
        }
