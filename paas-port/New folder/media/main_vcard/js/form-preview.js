...
// Don't let the links seduce you!
view.find('a').attr('target', '_blank');

view.preview({
  form: 'form',
  init: function (preview) {
      preview.addProcessor('url', function (name, value) {
          return value.toLowerCase().replace(/[^\w\d\-_]/g, '-');
      });

      preview.addProcessor('date', function (name, value) {
